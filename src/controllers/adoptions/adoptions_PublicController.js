...
class Adoptions_PublicController {
  ...
  // Método para atualizar uma adoção existente
  static async update(req, res) {
    try {
      const id = req.params.id; // Pega o ID da URL
      await AdoptionsService.update(id, req.body); // Chama o service para atualizar
  ...
  // Método para deletar uma adoção
  static async delete(req, res) {
    try {
      const id = req.params.id; // Pega o ID da URL
      await AdoptionsService.delete(id); // Chama o service para deletar
      res.json({ message: 'Adoção deletada com sucesso.' });
    } catch (error) {
      res.status(400).json({ error: error.message }); // Retorna erro se adoção não encontrada
    }
  }
}
